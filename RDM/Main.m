...
%[screenPointer,rect] = Screen('OpenWindow', max(Screen('Screens')), [], screenRect);
...
fileName        = strcat('../data/',subName,'_block_',block,'.mat');
...
condition           = condition(Shuffle(1:num_trial),:);      %Shuffle rows                               o f   t h                                           e   m a t r i x   t o       g   e   n e         r a t e           r a n   d o                            m trials
...
for i = 1 : num_trial
    ...
    if complete 
        ...
        if strcmp(KbName(keyCode1) ,'RightArrow')
            rec(i,1) = 0;
        else
            rec(i,1) = 1;
        end
...
txt = 'Finished';
Screen('DrawText', scr.wptr, txt, fp_loc(1)-70, fp_loc(2), [255 10 10]);
